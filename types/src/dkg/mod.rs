...
fn is_node_family_committee_member(addr: AccountAddress, dealer_committee: DkgCommittee, random_seed: Vec<u8>) -> bool {
    ...
    = get_family_node_indices(dealer_committee.committee.len() as u32, random_seed);
...
fn get_signer_bls_keys_from_indices(dealer_committee: DkgCommittee, signers: Vec<u32>, random_seed: Vec<u8>) -> anyhow::Result<Vec<PublicKey>>{

    let committee = dealer_committee.committee;
    let dealer_clan_committee_indices = get_clan_node_indices(committee.len() as u32, random_seed);
