...
use aptos_gas_schedule::gas_params::natives::aptos_framework::{
    RLP_ENCODE_DECODE_BASE, RLP_ENCODE_DECODE_PER_BYTE, UTIL_FROM_BYTES_BASE,
    UTIL_FROM_BYTES_PER_BYTE,
};
...
macro_rules! rlp_encode_scalar {
    ($context:expr, $args:ident, $rust_ty:ty, $num_bytes:expr) => {{
        ...
        $context.charge(
            RLP_ENCODE_DECODE_BASE + RLP_ENCODE_DECODE_PER_BYTE * NumBytes::new($num_bytes),
        )?;
    ...
    match inner_ty {
        ...
        Type::Address => {
            ...
            match rlp::decode::<Vec<u8>>(&encoded_data) {
                Ok(addr_bytes) => match AccountAddress::from_bytes(addr_bytes) {
...
fn native_rlp_encode_list_scalar(
    ...
    context.charge(
        RLP_ENCODE_DECODE_BASE + RLP_ENCODE_DECODE_PER_BYTE * NumBytes::new(total_data_bytes),
    )?;
    ...
    match inner_ty {
        ...
        Type::Address => {
            ...
            let data: Vec<AccountAddress> =
                bcs::from_bytes(&data).map_err(|_| SafeNativeError::Abort {
...
fn native_rlp_encode_list_byte_array(
    ...
    let data: Vec<Vec<u8>> = bcs::from_bytes(&data).map_err(|_| SafeNativeError::Abort {
        abort_code: E_ENCODE_FAILURE,
    })?;
...
fn native_rlp_decode_list_byte_array(
    ...
    match rlp.as_list::<Vec<u8>>() {
        Ok(decoded) => {
            ...
            let serialized_data = bcs::to_bytes(&decoded).map_err(|_| SafeNativeError::Abort {
                abort_code: E_DECODE_FAILURE,
            })?;
...
pub fn make_all(
    ...
    natives.extend([
        ...
        (
            ...
            native_rlp_encode_list_scalar as RawSafeNative,
        ),
        (
            "native_rlp_decode_list_scalar",
            native_rlp_decode_list_scalar as RawSafeNative,
        ),
        (
            "native_rlp_encode_list_byte_array",
            native_rlp_encode_list_byte_array as RawSafeNative,
        ),
        (
            "native_rlp_decode_list_byte_array",
            native_rlp_decode_list_byte_array as RawSafeNative,
        ),
    ]);
