...
module std::test_leader_ban_registry {
    ...
    fun test_ban_registy_e2e(
        ...
        setup_staking_modules(sender, validator_1, validator_2, validator_3, validator_4);
        ...
        assert!(signer::address_of(validator_2) == leader_ban_registry::get_pool_address_from_vp(vector::borrow(&ban_registry, 0)), 3 );
        ...
        let ban_registry = leader_ban_registry::get_ban_registry();
        let vp = vector::borrow(&ban_registry, 0);
        assert!(signer::address_of(validator_1) == leader_ban_registry::get_pool_address_from_vp(vp), 6 );

        leader_ban_registry::update_ban_registry(0, 12 + (4 * 1), option::some(1), vector[0]);
        let ban_registry = leader_ban_registry::get_ban_registry();
        let vp = vector::borrow(&ban_registry, 0);
        assert!(leader_ban_registry::get_consecutive_count_from_vp(vp) == 1, 8 );
        assert!(vector::length(&ban_registry) == 1, 9 );

        leader_ban_registry::update_ban_registry(0, 12 + (4 * 2), option::some(1), vector[0]);
        let ban_registry = leader_ban_registry::get_ban_registry();
        let vp = vector::borrow(&ban_registry, 0);
        assert!(leader_ban_registry::get_consecutive_count_from_vp(vp) == 2, 8 );
        assert!(vector::length(&ban_registry) == 1, 9 );

        leader_ban_registry::update_ban_registry(0, 12 + (4 * 3), option::some(1), vector[0]);
        let ban_registry = leader_ban_registry::get_ban_registry();
        let vp = vector::borrow(&ban_registry, 0);
        assert!(leader_ban_registry::get_consecutive_count_from_vp(vp) == 3, 8 );
        assert!(vector::length(&ban_registry) == 1, 9 );

        leader_ban_registry::update_ban_registry(0, 12 + (4 * 4), option::some(1), vector[0]);
        let ban_registry = leader_ban_registry::get_ban_registry();
        let vp = vector::borrow(&ban_registry, 0);
        ...
        leader_ban_registry::update_ban_registry(0, 12 + (4 * 5), option::some(1), vector[0]);
        let ban_registry = leader_ban_registry::get_ban_registry();
        ...
        leader_ban_registry::update_ban_registry(0, 33 + (4 * 1), option::some(2), vector[0,1]);
        leader_ban_registry::update_ban_registry(0, 33 + (4 * 2), option::some(2), vector[0,1]);
