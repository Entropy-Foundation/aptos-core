...
module supra_framework::test_leader_ban_registry_config {
    ...
    #[test(sender = @0xdead)]
    #[expected_failure(abort_code=0x50003, location=supra_framework::system_addresses)] // signer is not valid
    fun test_signer(sender: &signer) {
        let ban_registry_param = leader_ban_registry_config::get_test_ban_registry_params_v0();
    ...
    #[expected_failure(abort_code=0x10001, location=supra_framework::leader_ban_registry_config)] // Invalid config bytes
    ...
    #[test(sender = @supra_framework)]
    #[expected_failure(abort_code=0x10003, location=supra_framework::leader_ban_registry_config)] // Invalid version bytes
    ...
    public fun test_init_config_value(sender: &signer) {
        ...
        assert!(leader_ban_registry_config::check_ban_registry_params_v0_exist(sender), 2);
    }

    #[test(sender = @supra_framework)]
    #[expected_failure(abort_code=0x80004, location=supra_framework::leader_ban_registry_config)] // Already initialised
    ...
    fun test_on_epoch_change(sender: &signer) {
        ...
        let ban_registry_param = leader_ban_registry_config::get_custom_ban_registry_params_v0(
        ...
        leader_ban_registry_config::set_for_next_epoch(sender, config_bytes, 0);
        leader_ban_registry_config::on_new_epoch(sender);
        assert!(leader_ban_registry_config::get_initial_elections_denied()==updated_initial_e_denied, 3);
        assert!(leader_ban_registry_config::get_max_elections_denied()==updated_max_e_denied, 4);
        assert!(leader_ban_registry_config::get_minimum_unbanned_proposers()==updated_minimum_u_proposers, 5);
    }
}
