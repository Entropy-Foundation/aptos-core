spec supra_framework::vesting_without_staking {
    ...
    spec remaining_grant {
        ...
        aborts_if !simple_map::spec_contains_key(
            global<VestingContract>(vesting_contract_address).shareholders,
            shareholder_address
        );
    ...
    spec vesting_contracts {
        ...
        ensures !exists<AdminStore>(admin) ==>
            result == vector::empty<address>();
        // ensure that the vesting contracts are returned if the admin exists
        ensures exists<AdminStore>(admin) ==>
            result == global<AdminStore>(admin).vesting_contracts;
    }
    ...
    spec vesting_schedule {
        ...
        ensures result
            == global<VestingContract>(vesting_contract_address).vesting_schedule;
    }
    ...
    spec vest_transfer {
        ...
        let amount = min(
            ...
            fixed_point32::spec_multiply_u64(vesting_record.init_amount, vesting_fraction)
        );
        ...
        ensures beneficiary != address_from ==>
            (
                coin::balance<SupraCoin>(beneficiary)
                    == old(coin::balance<SupraCoin>(beneficiary)) + amount
                    && coin::balance<SupraCoin>(address_from)
                        == old(coin::balance<SupraCoin>(address_from)) - amount
            );
    }
    ...
    spec remove_shareholder {
        ...
        let post balance_post = coin::balance<SupraCoin>(
            vesting_contract_post.withdrawal_address
        );
        let shareholder_amount = simple_map::spec_get(
            vesting_contract.shareholders, shareholder_address
        ).left_amount;
        // ensure that `withdrawal address` receives the `shareholder_amount`
        ensures vesting_contract_post.withdrawal_address
            != vesting_contract.signer_cap.account ==>
            balance_post == balance_pre + shareholder_amount;
        // ensure that `shareholder_address` is indeed removed from the contract
        ensures !simple_map::spec_contains_key(
            vesting_contract_post.shareholders, shareholder_address
        );
        // ensure that beneficiary doesn't exist for the corresponding shareholder
        ensures !simple_map::spec_contains_key(
            vesting_contract_post.beneficiaries, shareholder_address
        );
    }
    ...
    spec admin_withdraw {
        ...
        let balance_pre = coin::balance<SupraCoin>(vesting_contract.withdrawal_address);
        let post balance_post = coin::balance<SupraCoin>(
            vesting_contract.withdrawal_address
        );
        let post balance_contract = coin::balance<SupraCoin>(contract_address);
        aborts_if !(
            global<VestingContract>(contract_address).state == VESTING_POOL_TERMINATED
        );
    ...
    spec set_beneficiary {
        ...
        ensures simple_map::spec_get(vesting_contract_post.beneficiaries, shareholder)
            == new_beneficiary;
    }
    ...
    spec get_beneficiary {
        ...
        ensures simple_map::spec_contains_key(contract.beneficiaries, shareholder) ==>
            result == simple_map::spec_get(contract.beneficiaries, shareholder);
