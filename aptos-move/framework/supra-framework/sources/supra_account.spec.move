spec supra_framework::supra_account {
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        aborts_if exists<coin::CoinStore<SupraCoin>>(to)
            && global<coin::CoinStore<SupraCoin>>(to).frozen;
        /// [high-level-req-5]
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<SupraCoin>>(account_addr_source).coin.value
                < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<SupraCoin>>(recipients[i])
                && global<coin::CoinStore<SupraCoin>>(recipients[i]).frozen;
