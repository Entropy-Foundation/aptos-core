...
module std::dkg_committee {
    ...
    public fun new_dkg_node_config(
        addr: address, identity: vector<u8>, dkg_pubkey: vector<u8>
    ): DkgNodeConfig {
        DkgNodeConfig { addr, identity, dkg_pubkey }
    }
    ...
    public fun new_dkg_committee(
        committee: vector<DkgNodeConfig>, threshold_type: CertificateThresholdType
    ): DkgCommittee {
        assert!(vector::length(&committee) > 0, EINVALID_DKG_COMMITTEE_SIZE);
        DkgCommittee { committee, threshold_type }
    }

    public fun new_dkg_committee_from_validator_consensus_info(
        validator_committee: vector<ValidatorConsensusInfo>,
        threshold_type: CertificateThresholdType
    ): DkgCommittee {
        ...
        DkgCommittee { committee: dkg_committee, threshold_type }
    }

    public fun new_receiver_committee(
        is_resharing: bool,
        dkg_threshold_type: CertificateThresholdType,
        committee: DkgCommittee
    ): ReceiverCommittee {
    ...
    public fun new_dkg_committee_output(
        threshold_type: u8, keys: vector<vector<u8>>
    ): DkgCommitteeOutput {
        DkgCommitteeOutput { threshold_type, keys }
    }

    /// Get threshold type from DkgCommitteeOutput
    public fun get_dkg_committee_output_threshold_type(
        output: &DkgCommitteeOutput
    ): u8 {
