spec supra_framework::stake {
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@supra_framework) ==>
        ...
        invariant [suspendable] chain_status::is_operating() ==>
            exists<ValidatorPerformance>(@supra_framework);
        invariant [suspendable] chain_status::is_operating() ==>
            exists<ValidatorSet>(@supra_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (
            reconfiguration_state::spec_is_in_progress()
                && old(exists<ValidatorSet>(@supra_framework))
        ) ==>
            old(global<ValidatorSet>(@supra_framework))
                == global<ValidatorSet>(@supra_framework);
    }
    ...
    spec initialize_validator(
        ...
        aborts_if exists<AllowedValidators>(@supra_framework)
    ...
    spec withdraw(owner: &signer, withdraw_amount: u64) {
        ...
        aborts_if bool_find_validator
            && !exists<timestamp::CurrentTimeMicroseconds>(@supra_framework);
        let new_withdraw_amount_1 = min(
            withdraw_amount, stake_pool.inactive.value
        ...
        ensures bool_find_validator
            ...
            && exists<coin::CoinStore<SupraCoin>>(addr) ==>
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        include (rewards_amount > 0) ==>
            coin::CoinAddAbortsIf<SupraCoin> { amount: amount };
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate =
                global<staking_config::StakingRewardsConfig>(@supra_framework).rewards_rate;
