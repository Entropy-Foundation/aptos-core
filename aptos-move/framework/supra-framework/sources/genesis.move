module supra_framework::genesis {
    ...
    fun initialize(
        ...
        // to use this account.
        let (supra_framework_account, supra_framework_signer_cap) =
            account::create_framework_reserved_account(@supra_framework);
        ...
        supra_governance::store_signer_cap(
        ...
        while (!vector::is_empty(&framework_reserved_addresses)) {
            ...
            supra_governance::store_signer_cap(
                &supra_framework_account, address, framework_signer_cap
            );
        };
        ...
        timestamp::set_time_has_started(
            &supra_framework_account, genesis_timestamp_in_microseconds
        );
    }
    ...
    fun initialize_core_resources_and_supra_coin(
        supra_framework: &signer, core_resources_auth_key: vector<u8>
    ) {
        let (burn_cap, mint_cap) = supra_coin::initialize(supra_framework);
        coin::create_coin_conversion_map(supra_framework);
        coin::create_pairing<SupraCoin>(supra_framework);
        ...
        supra_coin::configure_accounts_for_test(
    ...
    fun initialize_evm_genesis_config(
        supra_framework: &signer, evm_genesis_config: vector<u8>
    ) {
    ...
    fun initialize_leader_ban_registry_config(
        supra_framework: &signer, leader_ban_registry_config: vector<u8>
    ) {
        leader_ban_registry_config::initialize(
            supra_framework, leader_ban_registry_config
        );
        leader_ban_registry::initialize_leader_ban_registry(supra_framework);
    }

    fun create_accounts(
        supra_framework: &signer, accounts: vector<AccountMap>
    ) {
    ...
    fun create_account(
        supra_framework: &signer, account_address: address, balance: u64
    ): signer {
    ...
    fun create_multisig_account_with_balance(
        ...
        assert!(
            account::exists_at(owner),
            error::invalid_argument(EACCOUNT_DOES_NOT_EXIST)
        );
        assert!(
            vector::all(
                &additional_owners,
                |ao_addr| { account::exists_at(*ao_addr) }
            ),
            error::invalid_argument(EACCOUNT_DOES_NOT_EXIST)
        );
    ...
    fun create_initialize_validators(
        supra_framework: &signer, validators: vector<ValidatorConfiguration>
    ) {
        ...
        create_initialize_validators_with_commission(
            supra_framework, false, validators_with_commission
        );
    }
    ...
    fun create_pbo_delegation_pool(
        pbo_delegator_config: &PboDelegatorConfiguration, delegation_percentage: u64
    ) {
        assert!(
            delegation_percentage != 0 && delegation_percentage <= 100,
            error::invalid_argument(EPERCENTAGE_INVALID)
        ...
        let pool_address =
            pbo_delegation_pool::get_owned_pool_address(
                pbo_delegator_config.delegator_config.owner_address
            );
    ...
    fun assert_validator_addresses_check(
    ...
    fun create_vesting_without_staking_pools(
        ...
        vector::for_each_ref(
            ...
            |pool_config| {
                ...
                assert!(
                    schedule_length != 0,
                    error::invalid_argument(EVESTING_SCHEDULE_IS_ZERO)
                );
                assert!(
                    pool_config.vesting_denominator != 0,
                    error::invalid_argument(EDENOMINATOR_IS_ZERO)
                );
                assert!(
                ...
                let vesting_schedule =
                    vesting_without_staking::create_vesting_schedule(
                        schedule,
                ...
                while (j < num_shareholders) {
                    ...
                    assert!(
                        !vector::contains(&unique_accounts, &shareholder),
                        error::already_exists(EDUPLICATE_ACCOUNT)
                    );
                    ...
                    let amount_to_extract =
                        (amount * (pool_config.vpool_locking_percentage as u64)) / 100;
                    let coin_share =
                        coin::withdraw<SupraCoin>(
    ...
    fun initialize_for_verification(
        ...
        features::change_feature_flags_for_verification(
            supra_framework, vector[1, 2, 11], vector[]
        );
    ...
    fun generate_multisig_account(
        owner: &signer, addition_owner: vector<address>
    ): address {
    ...
    fun test_create_root_account(supra_framework: &signer) {
        ...
        features::change_feature_flags_for_testing(
            supra_framework, vector[feature], vector[]
        );
        ...
        assert!(
            primary_fungible_store::primary_store_exists(@core_resources, sup_metadata),
            2
        );
    ...
    fun test_create_pbo_delegation_pool(supra_framework: &signer) {
        ...
        let multisig =
            generate_multisig_account(
                &account::create_signer_for_test(owner), vector[@0x12134]
            );
        let pbo_delegator_config = PboDelegatorConfiguration {
    ...
    fun test_create_pbo_delegation_pools(supra_framework: &signer) {
        ...
        let multisig1 =
            generate_multisig_account(
                &account::create_signer_for_test(owner1), vector[@0x121342]
            );
        ...
        while (j < vector::length(&delegator_address2)) {
            ...
            create_account(
                supra_framework,
                *vector::borrow(&delegator_address2, j),
                *bal
            );
            vector::push_back(
                &mut delegator_stakes2,
                (*bal) * delegation_percentage / 100
            );
            j = j + 1;
        };
        ...
        let multisig2 =
            generate_multisig_account(
                &account::create_signer_for_test(owner2), vector[@0x121347]
            );
    ...
    #[test(
        supra_framework = @0x1, owner1 = @0x1234, owner2 = @0x2345, owner3 = @0x3456
    )]
    ...
    fun test_create_multisig_account_with_schema(
        ...
        assert!(
            vector::all(
                &vec_addr,
                |elem| { multisig_account::num_signatures_required(*elem) == 2 }
            ),
            1
        );
        ...
        assert!(
            vector::all(
                &vec_addr,
                |elem| {
                    coin::balance<SupraCoin>(*elem) == balance
                }
            ),
            2
        );
    ...
    #[test(supra_framework = @0x1)]
    fun test_create_vesting_without_staking_pools(
        supra_framework: &signer
    ) {
