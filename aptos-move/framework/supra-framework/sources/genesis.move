module supra_framework::genesis {
    ...
    fun create_multisig_account_with_balance(
        ...
        assert!(
            account::exists_at(owner), error::invalid_argument(EACCOUNT_DOES_NOT_EXIST)
        );
        assert!(
    ...
    fun create_vesting_without_staking_pools(
        ...
        vector::for_each_ref(
            ...
            |pool_config| {
                ...
                assert!(
                    schedule_length > 0, error::invalid_argument(
                        EVESTING_SCHEDULE_IS_ZERO
                    )
                );
                assert!(
                ...
                let sum = vector::fold(pool_config.vesting_numerators, 0, |acc, x| acc
                    + x);
                ...
                while (j < num_shareholders) {
                    ...
                    let coin_share =
                        coin::withdraw<SupraCoin>(&shareholder_signer, amount_to_extract);
    ...
    fun generate_multisig_account(
        ...
        multisig_account::create_with_owners(
            owner, addition_owner, 2, vector[], vector[], 300
        );
        multisig_addr
    }
    ...
    fun test_create_pbo_delegation_pools(supra_framework: &signer) {
        ...
        while (j < vector::length(&delegator_address2)) {
            ...
            vector::push_back(
                &mut delegator_stakes2, (*bal) * delegation_percentage / 100
            );
            j = j + 1;
        };
    ...
    fun test_create_multisig_account_with_schema(
        ...
        assert!(
            vector::all(
                &vec_addr, |elem| { multisig_account::num_signatures_required(*elem)
                    == 2 }
            ),
            1
        );
