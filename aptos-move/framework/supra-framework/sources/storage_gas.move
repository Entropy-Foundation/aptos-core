...
module supra_framework::storage_gas {
    ...
    public(friend) fun set_config(
        supra_framework: &signer, config: StorageGasConfig
    ) acquires StorageGasConfig {
        system_addresses::assert_supra_framework(supra_framework);
        *borrow_global_mut<StorageGasConfig>(@supra_framework) = config;
    }
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@supra_framework);
        let gas = borrow_global_mut<StorageGas>(@supra_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@supra_framework);
