module supra_framework::transaction_validation {
    ...
    const PROLOGUE_ENONCE_ALREADY_USED: u64 = 1013;
    const PROLOGUE_ETRANSACTION_EXPIRATION_TOO_FAR_IN_FUTURE: u64 = 1014;
    ...
    fun automated_transaction_epilogue(
        ...
        let addr = signer::address_of(&account);
        epilogue_gas_payer_only_extended(addr, storage_fee_refunded, txn_gas_price, txn_max_gas_units, gas_units_remaining, false);
    }
    ...
    fun epilogue_gas_payer_extended(
        ...
        epilogue_gas_payer_only_extended(
            gas_payer,
            storage_fee_refunded,
            txn_gas_price,
            txn_max_gas_units,
    ...
    // Once the transaction_simulation_enhancement feature is enabled, the Aptos VM will invoke this function instead.
    // Eventually, this function will be consolidated with the original function once the feature is fully enabled.
    fun epilogue_gas_payer_only_extended(
        gas_payer: address,
        storage_fee_refunded: u64,
        txn_gas_price: u64,
        txn_max_gas_units: u64,
