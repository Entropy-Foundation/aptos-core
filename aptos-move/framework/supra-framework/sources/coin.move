...
module supra_framework::coin {
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@supra_framework)
            ...
            let map =
                &borrow_global<CoinConversionMap>(@supra_framework).coin_to_fungible_asset_map;
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(
        ...
        assert!(
            exists<CoinConversionMap>(@supra_framework),
    ...
    public fun allow_supply_upgrades(
        supra_framework: &signer, allowed: bool
    ) acquires SupplyConfig {
        system_addresses::assert_supra_framework(supra_framework);
        let allow_upgrades =
            &mut borrow_global_mut<SupplyConfig>(@supra_framework).allow_upgrades;
        ...
        if (exists<CoinStore<CoinType>>(account_addr)) {
            ...
            if (option::is_some(&metadata)
                ...
                primary_fungible_store::deposit(
                    account_addr, coin_to_fungible_asset_internal(coin)
                );
            } else {
    ...
    inline fun migrated_primary_fungible_store_exists(
        ...
        fungible_asset::store_exists(primary_store_address)
            && (
                ...
                features::new_accounts_default_to_fa_supra_store_enabled()
    ...
    public(friend) fun initialize_with_parallelizable_supply_with_limit<CoinType>(
        ...
        system_addresses::assert_supra_framework(account);
        initialize_internal_with_limit(
            account, name, symbol, decimals, monitor_supply, true, limit
        )
    }
    ...
    fun initialize_internal_with_limit<CoinType>(
        ...
        let coin_info = CoinInfo<CoinType> {
            ...
            decimals,
            supply: if (monitor_supply) {
                option::some(optional_aggregator::new(limit, parallelizable))
            } else {
                option::none()
            }
        };
    ...
    fun test_is_account_registered(
        ...
        primary_fungible_store::deposit(
            bob_addr, coin_to_fungible_asset_internal(mint<FakeMoney>(100, &mint_cap))
        );
