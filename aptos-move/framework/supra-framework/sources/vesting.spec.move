spec supra_framework::vesting {
    ...
    spec create_vesting_contract {
        ...
        pragma verify = false;
        /// [high-level-req-10]
        aborts_if withdrawal_address == @supra_framework || withdrawal_address == @vm_reserved;
        aborts_if !exists<account::Account>(withdrawal_address);
        aborts_if !exists<coin::CoinStore<SupraCoin>>(withdrawal_address);
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<SupraCoin>>(resource_addr) && !aptos_std::type_info::spec_is_struct<SupraCoin>();
        aborts_if !exists<coin::CoinStore<SupraCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 > MAX_U64;
        aborts_if !exists<coin::CoinStore<SupraCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        ensures exists<account::Account>(resource_addr) && post_acc.authentication_key == account::ZERO_AUTH_KEY &&
                exists<coin::CoinStore<SupraCoin>>(resource_addr);
    ...
    spec withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<SupraCoin> {
        // TODO: Calls `staking_contract::distribute` which is not verified.
        pragma verify = false;
    ...
    spec schema DistributeInternalAbortsIf {
        ...
        aborts_if !exists<stake::ValidatorSet>(@supra_framework);
        let validator_set = global<stake::ValidatorSet>(@supra_framework);
