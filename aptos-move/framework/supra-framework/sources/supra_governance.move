...
module supra_framework::supra_governance {
    ...
    public fun store_signer_cap(
        supra_framework: &signer, signer_address: address, signer_cap: SignerCapability
        ...
        let signer_caps =
            &mut borrow_global_mut<GovernanceResponsbility>(@supra_framework).signer_caps;
    ...
    fun initialize(
        ...
        assert!(
            vector::length(&voters) >= min_voting_threshold
                && min_voting_threshold > vector::length(&voters) / 2,
        ...
        move_to(
            supra_framework,
            SupraGovernanceConfig { voting_duration_secs, min_voting_threshold, voters }
        );
        move_to(
            supra_framework,
            SupraGovernanceEvents {
                create_proposal_events: account::new_event_handle<SupraCreateProposalEvent>(
                    supra_framework
                ),
                update_config_events: account::new_event_handle<SupraUpdateConfigEvent>(
                    supra_framework
                ),
    ...
    public fun update_supra_governance_config(
        ...
        let supra_governance_config =
            borrow_global_mut<SupraGovernanceConfig>(@supra_framework);
        ...
        if (std::features::module_event_migration_enabled()) {
            event::emit(
                SupraUpdateConfig { min_voting_threshold, voting_duration_secs, voters }
            )
        };
        ...
        event::emit_event<SupraUpdateConfigEvent>(
            ...
            SupraUpdateConfigEvent { min_voting_threshold, voting_duration_secs, voters }
        );
    }
    ...
    public fun supra_create_proposal_v2_impl(
        ...
        let supra_governance_config =
            borrow_global<SupraGovernanceConfig>(@supra_framework);

        assert!(
            vector::contains(&supra_governance_config.voters, &proposer_address),
            error::permission_denied(EACCOUNT_NOT_AUTHORIZED)
        );

        let proposal_expiration =
            timestamp::now_seconds() + supra_governance_config.voting_duration_secs;
        ...
        let proposal_id =
            multisig_voting::create_proposal_v2(
                proposer_address,
                @supra_framework,
    ...
    public entry fun supra_vote(
        voter: &signer, proposal_id: u64, should_pass: bool
    ...
    fun supra_vote_internal(
        ...
        assert!(
            vector::contains(
                &supra_governance_config.voters, &signer::address_of(voter)
            ),
            error::permission_denied(EACCOUNT_NOT_AUTHORIZED)
        );
        ...
        let proposal_expiration =
            multisig_voting::get_proposal_expiration_secs<GovernanceProposal>(
                @supra_framework, proposal_id
            );
        assert!(
            timestamp::now_seconds() <= proposal_expiration,
            error::invalid_argument(EPROPOSAL_IS_EXPIRE)
        );
        ...
        if (std::features::module_event_migration_enabled()) {
            event::emit(SupraVote { proposal_id, voter: voter_address, should_pass });
        };
        let events = borrow_global_mut<SupraGovernanceEvents>(@supra_framework);
        event::emit_event<SupraVoteEvent>(
            &mut events.vote_events,
            SupraVoteEvent { proposal_id, voter: voter_address, should_pass }
        );

        let proposal_state =
            multisig_voting::get_proposal_state<GovernanceProposal>(
                @supra_framework, proposal_id
            );
    ...
    public entry fun add_supra_approved_script_hash_script(
        proposal_id: u64
    ...
    public fun add_supra_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes =
            borrow_global_mut<ApprovedExecutionHashes>(@supra_framework);
        ...
        let execution_hash =
            multisig_voting::get_execution_hash<GovernanceProposal>(
                @supra_framework, proposal_id
            );
    ...
    public fun resolve_supra_multi_step_proposal(
        ...
        multisig_voting::resolve_proposal_v2<GovernanceProposal>(
    ...
    public fun remove_supra_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            multisig_voting::is_resolved<GovernanceProposal>(
                @supra_framework, proposal_id
            ),
        ...
        let approved_hashes =
            &mut borrow_global_mut<ApprovedExecutionHashes>(@supra_framework).hashes;
    ...
    public entry fun reconfigure(supra_framework: &signer) {
        ...
        // The block executor should seed the randomness config with the digest of the last
        // block of the first epoch in which the DKG is enabled, so the second conjunct should
        // always be true once the first is true. We keep it for safety in case of regression.
        if (features::supra_dkg_enabled() && randomness_config::enabled()) {
    ...
    public fun toggle_features(
        supra_framework: &signer, enable: vector<u64>, disable: vector<u64>
    ) {
        system_addresses::assert_supra_framework(supra_framework);
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(
            supra_coin::has_mint_capability(core_resources),
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility =
            borrow_global<GovernanceResponsbility>(@supra_framework);
        let signer_cap =
    ...
    public entry fun supra_create_proposal_for_test(
        ...
        if (multi_step) {
            supra_create_proposal_v2(proposer, execution_hash, b"", b"", true);
        } else {
            supra_create_proposal(proposer, execution_hash, b"", b"");
        };
    }
    ...
    public fun supra_resolve_proposal_for_test(
        ...
        if (multi_step) {
            ...
            if (finish_multi_step_execution) {
                resolve_supra_multi_step_proposal(
                    proposal_id, signer_address, vector::empty<u8>()
                )
            } else {
                resolve_supra_multi_step_proposal(
                    proposal_id, signer_address, execution_hash
                )
            }
        } else {
    ...
    public entry fun test_voting_generic(
        ...
        supra_test_resolving_proposal_generic(
            supra_framework, use_generic_resolve_function, execution_hash
        );
    }
    ...
    public entry fun supra_test_resolving_proposal_generic(
        ...
        let proposal_state =
            multisig_voting::get_proposal_state<GovernanceProposal>(
                signer::address_of(supra_framework), 0
            );
        ...
        let approved_hashes =
            borrow_global<ApprovedExecutionHashes>(@supra_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account =
            supra_resolve_proposal_for_test(
                0,
                @supra_framework,
        ...
        assert!(
            multisig_voting::is_resolved<GovernanceProposal>(@supra_framework, 0), 2
        );
        let approved_hashes =
            borrow_global<ApprovedExecutionHashes>(@supra_framework).hashes;
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            multisig_voting::resolve_proposal_v2<GovernanceProposal>(
                @supra_framework, 0, next_execution_hash
            );
            assert!(
                multisig_voting::is_resolved<GovernanceProposal>(@supra_framework, 0),
                0
            );
            if (vector::length(&next_execution_hash) == 0) {
    ...
    public entry fun test_cannot_double_vote(
        ...
        let voters = vector[
            ...
            signer::address_of(voter_2)
        ];
        supra_setup_voting(supra_framework, voters);

        supra_create_proposal(proposer, b"random-test", b"", b"");
    ...
    public fun supra_setup_voting(
        supra_framework: &signer, voters: vector<address>
    ) acquires GovernanceResponsbility {
