module supra_framework::account {
    ...
    public(friend) fun initialize(supra_framework: &signer) {
        ...
        move_to(supra_framework, OriginatingAddress { address_map: table::new() });
    }
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map =
            &mut borrow_global_mut<OriginatingAddress>(@supra_framework).address_map;
    ...
    public entry fun mock_sequence_numbers_source_code() acquires Account {
        let addr: address = @0x1234; // Define test address
        create_account(addr); // Initialize account resource
        // Assert sequence number intializes to 0
        assert!(borrow_global<Account>(addr).sequence_number == 0, 0);
    ...
    public entry fun test_create_account_twice_should_fail(
    ...
    public entry fun test_create_account_with_core_resource_address_should_fail(
        alice_address: address
    ) {
        create_account(alice_address);
    ...
    public entry fun test_create_framework_reserved_account_with_core_resource_address_should_fail(
        alice_address: address
    ) {
