spec supra_framework::transaction_fee {
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==>
            exists<SupraCoinCapabilities>(@supra_framework)
                || exists<SupraFABurnCapabilities>(@supra_framework);
    }
    ...
    spec upgrade_burn_percentage(supra_framework: &signer, new_burn_percentage: u8) {
        ...
        ensures exists<CollectedFeesPerBlock>(@supra_framework) ==>
            global<CollectedFeesPerBlock>(@supra_framework).burn_percentage
                == new_burn_percentage;
    }
    ...
    spec register_proposer_for_fee_collection(proposer_addr: address) {
        ...
        ensures is_fees_collection_enabled() ==>
            option::spec_borrow(global<CollectedFeesPerBlock>(@supra_framework).proposer) ==
                proposer_addr;
    }
    ...
    spec burn_coin_fraction(coin: &mut Coin<SupraCoin>, burn_percentage: u8) {
        ...
        include amount_to_burn > 0 ==>
            coin::CoinSubAbortsIf<SupraCoin> { amount: amount_to_burn };
        ...
        requires (is_fees_collection_enabled() && option::is_some(maybe_supply)) ==>
            (
                aggregator::spec_aggregator_get_val(
                    global<CollectedFeesPerBlock>(@supra_framework).amount.value
    ...
    spec burn_fee(account: address, fee: u64) {
        ...
        aborts_if amount != 0
            && !(
                exists<CoinInfo<SupraCoin>>(aptos_addr)
    ...
    spec store_supra_coin_burn_cap(
        ...
        ensures exists<SupraFABurnCapabilities>(addr)
            || exists<SupraCoinCapabilities>(addr);
    }

    /// Ensure caller is admin.
    /// Aborts if `SupraCoinMintCapability` already exists.
    spec store_supra_coin_mint_cap(
