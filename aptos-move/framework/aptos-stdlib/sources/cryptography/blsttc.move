...
// many things need to be update and implement properly, since it is just forked from bls12381 and modified for as per usecase for trials and move forward with work. 

module aptos_std::blsttc {
    use std::option::{Self, Option};
    #[test_only]
    use std::error::invalid_argument;

    /// The signature size, in bytes
    const SIGNATURE_SIZE: u64 = 48;

    /// The public key size, in bytes
    const PUBLIC_KEY_NUM_BYTES: u64 = 96;
    ...
    // public fun aggregate_pubkeys(public_keys: vector<PublicKey>): AggrPublicKey {
    ...
    public fun generate_keys(): (SecretKey, PublicKey) {
    ...
    /// Aborts if no public keys are given as input.
    native fun aggregate_pubkeys_internal(public_keys: vector<PublicKey>): (vector<u8>, bool);
    ...
    // #[test_only]
    // fun get_random_pk_with_pop(): PublicKey {
