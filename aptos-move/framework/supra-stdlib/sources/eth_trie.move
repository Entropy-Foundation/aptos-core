module supra_std::eth_trie {
    ...
    public fun verify_eth_trie_inclusion_proof(
        ...
        assert!(features::eth_trie_enabled(), EETH_TRIE_FEATURE_DISABLED);
    ...
    public fun native_verify_proof_eth_trie(
        root: vector<u8>,
        key: vector<u8>,
        proof: vector<vector<u8>>
    ): (bool, vector<u8>) {
        assert!(features::eth_trie_enabled(), EETH_TRIE_FEATURE_DISABLED);
        native_verify_proof_eth_trie_internal(root, key, proof)
    }

    //
    // Native functions
    //
    native fun native_verify_proof_eth_trie_internal(
        root: vector<u8>,
        key: vector<u8>,
        proof: vector<vector<u8>>
    ): (bool, vector<u8>);
    ...
    fun prepare_env(supra_framework: &signer) {
        let flag = vector[features::get_eth_trie_feature()];
