...
/// Create a new collection
pub fn supra_token_create_collection(
    description: Vec<u8>,
    max_supply: u64,
    name: Vec<u8>,
    uri: Vec<u8>,
    ...
    TransactionPayload::EntryFunction(EntryFunction::new(
        ModuleId::new(
            ...
            ident_str!("supra_token").to_owned(),
        ),
...
/// With an existing collection, directly mint a soul bound token into the recipient's account.
pub fn supra_token_mint_soul_bound(
    collection: Vec<u8>,
    description: Vec<u8>,
    name: Vec<u8>,
    uri: Vec<u8>,
...
mod decoder {
    ...
    pub fn supra_token_create_collection(
        ...
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(EntryFunctionCall::SupraTokenCreateCollection {
    ...
    pub fn supra_token_mint(payload: &TransactionPayload) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(EntryFunctionCall::SupraTokenMint {
    ...
    pub fn supra_token_mint_soul_bound(payload: &TransactionPayload) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(EntryFunctionCall::SupraTokenMintSoulBound {
                collection: bcs::from_bytes(script.args().get(0)?).ok()?,
                description: bcs::from_bytes(script.args().get(1)?).ok()?,
                name: bcs::from_bytes(script.args().get(2)?).ok()?,
                uri: bcs::from_bytes(script.args().get(3)?).ok()?,
...
static SCRIPT_FUNCTION_DECODER_MAP: once_cell::sync::Lazy<EntryFunctionDecoderMap> =
    once_cell::sync::Lazy::new(|| {
        ...
        map.insert(
            "supra_token_create_collection".to_string(),
            Box::new(decoder::supra_token_create_collection),
        );
        map.insert(
            "supra_token_mint".to_string(),
            Box::new(decoder::supra_token_mint),
        );
        map.insert(
            "supra_token_mint_soul_bound".to_string(),
            Box::new(decoder::supra_token_mint_soul_bound),
        );
        map
    });
